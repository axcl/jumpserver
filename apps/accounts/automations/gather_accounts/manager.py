...
class GatherAccountsManager(AccountBasePlaybookManager):
    def __init__(self, *args, **kwargs):
        ...
        self.asset_id_account_username_mapper = defaultdict(set)
    ...
    def generate_data(self, asset, result):
        ...
        for username, info in result.items():
            self.asset_id_account_username_mapper[str(asset.id)].add(username)
    ...
    def run(self, *args, **kwargs):
        ...
        if not self.asset_id_account_username_mapper or not recipients:
            return
        ...
        self.send_asset_account_change_info(users)

    def send_asset_account_change_info(self, users):
        asset_ids = self.asset_id_account_username_mapper.keys()
        assets = Asset.objects.filter(id__in=asset_ids)
        asset_id_map = {str(asset.id): asset for asset in assets}
        asset_qs = assets.values_list('id', 'accounts__username')
        system_asset_id_usernames_mapper = defaultdict(set)
        for asset_id, username in asset_qs:
            system_asset_id_usernames_mapper[str(asset_id)].add(username)

        asset_account_change_info = {}
        for asset_id, usernames in self.asset_id_account_username_mapper.items():
            system_usernames = system_asset_id_usernames_mapper.get(asset_id)
            ...
            asset_account_change_info[k] = {
                'add_usernames': ', '.join(add_usernames),
                'remove_usernames': ', '.join(remove_usernames),
            }

        if not asset_account_change_info:
            return

        for user in users:
            GatherAccountChangeMsg(
                user, asset_account_change_info
            ).publish_async()
